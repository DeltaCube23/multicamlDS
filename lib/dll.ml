...
let create_node a b c =
  { value = a; next = b; prev = c; lock = Mutex.create (); active = true }

let create key =
  let temp =
    { head = create_node key None None; tail = create_node key None None }
  in
...
let delete cur_node =
  ...
  let rec validate before after =
    ...
    if cur_node.active = false then (
      ...
      before == Option.get cur_node.prev && after == Option.get cur_node.next
    then (
