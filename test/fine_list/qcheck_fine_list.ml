  ...
  QCheck.
    [
      ...
      Test.make ~name:"duplicate add" (small_nat) (fun len ->
        ...
        let p2 = ref 0 in
        let producer1 = 
          Domain.spawn (fun () -> List.iter (fun ele -> if (Fine_list.add llist ele) then incr p1) lpush1)
        in
        let producer2 = 
          Domain.spawn (fun () -> List.iter (fun ele -> if (Fine_list.add llist ele) then incr p2) lpush2)
        in
      ...
      Test.make ~name:"duplicate remove" (small_nat) (fun len ->
        ...
        let c2 = ref 0 in
        let consumer1 = 
          Domain.spawn (fun () -> List.iter (fun ele -> if (Fine_list.remove llist ele) then incr c1) lpush1)
        in
        let consumer2 = 
          Domain.spawn (fun () -> List.iter (fun ele -> if (Fine_list.remove llist ele) then incr c2) lpush2)
        in
      ...
      Test.make ~name:"parallel add remove" (small_nat) (fun len ->
        ...
        let llist = Fine_list.create 0 in
        let producer1 = 
          Domain.spawn (fun () -> List.iter (fun ele -> ignore @@ Fine_list.add llist ele) lpush1)
        in
        let producer2 = 
          Domain.spawn (fun () -> List.iter (fun ele -> ignore @@ Fine_list.add llist ele) lpush2)
        in
        Domain.join producer1;
        Domain.join producer2;

        let consumer1 = 
          Domain.spawn (fun () -> List.iter (fun ele -> ignore @@ Fine_list.remove llist ele) lpush1)
        in
        let consumer2 = 
          Domain.spawn (fun () -> List.iter (fun ele -> ignore @@ Fine_list.remove llist ele) lpush2)
        in
        Domain.join consumer1;
        Domain.join consumer2;
        (* Testing property *)
