  ...
  QCheck.
    [
      ...
      Test.make ~count:1000 ~name:"push_pop_check_order" small_nat (fun len ->
          ...
          List.iter (fun ele -> ignore @@ Lockfree_pq.push queue ele) lpush;
  ...
  QCheck.
    [
      ...
      Test.make ~count:100 ~name:"double_add_remove" small_nat (fun nlen ->
          ...
          let producer1 =
            Domain.spawn (fun () ->
                List.iter (fun ele -> Lockfree_pq.push queue ele) lpush1)
          in
          let producer2 =
            Domain.spawn (fun () ->
                List.iter (fun ele -> Lockfree_pq.push queue ele) lpush2)
          in
      ...
      Test.make ~count:100 ~name:"add_double_remove" small_nat (fun nlen ->
          ...
          let queue = Lockfree_pq.create ~max_height:20 () in
          List.iter (fun ele -> Lockfree_pq.push queue ele) lpush;
          let c1 = ref 0 in
          let c2 = ref 0 in
          let consumer1 =
            Domain.spawn (fun () ->
